...
def open_create_issue_page(url, ischrome):
    url = url.replace(" ", "%20")
    url = url.replace("!", "%21")
    url = url.replace("#", "%23")
    url = url.replace("$", "%24")
...
def mi_by_ag_re(isad=True, ischrome=True):
    ...
    open_create_issue_page(site_url, ischrome)
